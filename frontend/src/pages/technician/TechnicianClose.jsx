...
import { 
  X, Clock, User, Calendar, MapPin, MessageSquare, Edit, Save, Search,
  Filter, ChevronDown, FileText, AlertCircle
} from 'lucide-react';
import { toast } from 'react-hot-toast';

function TechnicianClose({ tickets, setTickets, searchTerm, loading, fetchTickets }) {
  ...
  // Filter tickets assigned to current technician and status CLOSED
  const filteredTickets = tickets.filter(ticket => {
    ...
    return isAssignedToMe && matchesSearch && isClosed;
  });
  ...
  const handleResolutionUpdate = async (ticketId, notes) => {
    try {
      ...
      toast.error(`Error updating resolution notes: ${error.message}`);
        ...
        <div className="mt-4 p-3 bg-emerald-50 border border-emerald-200 rounded-lg">
          <div className="flex items-center justify-between mb-2">
            ...
            {editingResolution && selectedTicket?.id === ticket.id ? (
              <div className="flex items-center gap-2">
                <button
                  onClick={() => handleResolutionUpdate(ticket.id, resolutionNotes)}
          ...
          {editingResolution && selectedTicket?.id === ticket.id ? (
            ...
            <p className="text-sm text-emerald-700">{ticket.resolutionNotes || 'No resolution notes provided'}</p>
          )}
        </div>
      )}
    </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-slate-900">Closed Tickets</h2>
          <p className="text-slate-600 mt-1">Tickets that have been closed</p>
        </div>
        <div className="flex items-center gap-2">
          <X className="w-5 h-5 text-slate-600" />
          <span className="text-sm font-medium text-slate-700">
            {filteredTickets.length} closed ticket{filteredTickets.length !== 1 ? 's' : ''}
          </span>
        </div>
      </div>
      ...
      {filteredTickets.length === 0 && !loading && (
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-medium text-slate-900 mb-2">No closed tickets</h3>
          <p className="text-slate-600">You haven't closed any tickets yet</p>
        </div>
      )}
      ...
      {loading && (
        <div className="text-center py-12">
          ...
          <p className="text-slate-600 mt-4">Loading closed tickets...</p>
        </div>
      )}
                ...
                <div>
                  ...
                  <div className="mt-2 p-3 bg-emerald-50 border border-emerald-200 rounded-lg">
                    <p className="text-sm text-emerald-700">
                      {selectedTicket.resolutionNotes || 'No resolution notes provided'}
                    </p>
                  </div>
                </div>
              )}
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-end gap-3">
                ...
                <button
                  ...
                  disabled={!tempResolutionNotes.trim()}
                  className="px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed"
                >
...
export default TechnicianClose;
