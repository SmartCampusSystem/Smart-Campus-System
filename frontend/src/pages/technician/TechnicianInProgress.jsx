...
import { 
  Activity, Clock, User, Calendar, MapPin, MessageSquare, Edit, Save, X, Search,
  Filter, ChevronDown, FileText, AlertCircle
} from 'lucide-react';
import { toast } from 'react-hot-toast';

function TechnicianInProgress({ tickets, setTickets, searchTerm, loading, fetchTickets }) {
  ...
  // Filter tickets assigned to current technician and status IN_PROGRESS
  const filteredTickets = tickets.filter(ticket => {
    ...
    // Only show IN_PROGRESS tickets
    const isInProgress = ticket.status === 'IN_PROGRESS';
    
    return isAssignedToMe && matchesSearch && isInProgress;
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-slate-900">In Progress Tickets</h2>
          <p className="text-slate-600 mt-1">Tickets currently being worked on</p>
        </div>
        <div className="flex items-center gap-2">
      ...
      {filteredTickets.length === 0 && !loading && (
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-medium text-slate-900 mb-2">No in-progress tickets</h3>
          <p className="text-slate-600">You don't have any tickets currently being worked on</p>
        </div>
      )}
      ...
      {loading && (
        <div className="text-center py-12">
          ...
          <p className="text-slate-600 mt-4">Loading in-progress tickets...</p>
        </div>
      )}
